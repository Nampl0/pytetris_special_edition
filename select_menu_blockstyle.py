...
def select_blockstyle(screen, font):
    """
    Display block style selection menu.
    ...
    Returns:
        - "rotate" or "swap" based on selection
    """
    selected_style = None
    running = True
    ...
    while running:
        ...
        # Draw title
        title = font.render("SELECT SPACE BAR ACTION", True, constants.WHITE)
        ...
        def create_style_action(style):
            def action():
                nonlocal selected_style, running
                selected_style = style
                running = False
            return action

        # Draw buttons for each style
        styles = [("Rotate", "rotate"), ("Swap", "swap")]
        for i, (text, style) in enumerate(styles):
            ...
            draw_button(
                ...
                button_width,
                button_height,
                constants.PURPLE,  # default color
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                return "rotate"  # default to rotate if quit
